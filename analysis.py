...
zip_submitted = st.button("Submit ZIP Code")

if zip_submitted:
    #make API request to the Flask API
    ...
    if response.status_code == 200 and response.headers['content-type'] == 'application/json':
        ...
        st.subheader(f"ZIP Code: {zipcode}")
        st.subheader(f"Default Interest Rate: {data['default_interest_rate']}")
        st.subheader(f"State: {data['state']}")
        st.subheader(f"Tax Rate: {data['tax_rate']}")


        #extract tax rate and default_interest rate from the response
